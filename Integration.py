...
def delect_student():
    student_id = entry_id.get()
    cursor.execute('SELECT * from DB_studnt where db_student_id = ?',(student_id,))
    delect = cursor.fetchall()
    cursor.execute('DELETE * from DB_studnt where db_student_id = ?',(student_id,))
    print ('Following row is delete:', delete)
    conn.commit()

button_delete = tk.Button(root, text='Delete', command=delete_student)
bitton_delete.pack(pady=25)
